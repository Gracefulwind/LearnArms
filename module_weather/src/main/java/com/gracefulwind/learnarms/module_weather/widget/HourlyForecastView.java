...
import com.gracefulwind.learnarms.module_weather.app.entity.weather.WeatherHourly;

import java.util.ArrayList;
import java.util.List;

/**
 * @ClassName: HourlyForecastView
 * @Author: Gracefulwind
 * @CreateDate: 2020/6/2 10:13
 * @Description: ---------------------------
 * @UpdateUser:
 * @UpdateDate: 2020/6/2 10:13
 * @UpdateRemark:
 * @Version: 1.0
...
public class HourlyForecastView extends View {
    ...
    private List<WeatherHourly> forecastList;
    ...
    private void init(Context context) {
        ...
        TypefaceUtil.getDefaultTypeface();
    }
...
//		}
    ...
    public void setData(WeatherEntity weather) {
        ...
        if (this.forecastList == weather.getHourly()) {
//			percent = 0f;
            invalidate();
            return;
        }
        try {
            final List<WeatherHourly> w_hourlyForecast = weather.getHourly();
            if (w_hourlyForecast.size() == 0) {// 有可能为空
                return;
            }
            if (!TimeUtil.isToday(w_hourlyForecast.get(0).getTime())) {// 不是今天的数据
                return;
            }
            ...
            for (int i = 0; i < forecastList.size(); i++) {
                WeatherHourly forecast = forecastList.get(i);
